"""
    Зачет проводится отдельно в каждом классе. Победителями олимпиады
    становятся школьники, которые набрали наибольший балл среди всех
    участников в данном классе.
...
second = [0, 0, 0]
for line in infile:
    c, s = map(int, line.split()[-2:])
    if first[c - 9] < s:
        first[c - 9], second[c - 9] = s, first[c-9]
    elif second[c - 9] < s != first[c - 9]:
        second[c - 9] = s
